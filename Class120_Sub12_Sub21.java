...
class Class120_Sub12_Sub21 extends Class120_Sub12 {
	...
	static int menuMouseY;//TODO new name
	...
	static final void processSoundEffects() {
		for (int id = 0; id < Class150.soundEffectCount; id++) {
			...
			if (Class81.soundEffectDelays[id] < -10) {
				...
				for (int higherIds = id; higherIds < Class150.soundEffectCount; higherIds++) {
					Class120_Sub12_Sub16.soundEffectIds[higherIds] = Class120_Sub12_Sub16.soundEffectIds[higherIds + 1];
					Class109.soundEffects[higherIds] = Class109.soundEffects[higherIds + 1];
					LocType.soundEffectLoops[higherIds] = LocType.soundEffectLoops[higherIds + 1];
					Class81.soundEffectDelays[higherIds] = Class81.soundEffectDelays[higherIds + 1];
					Class174.SoundEffectCoordinates[higherIds] = Class174.SoundEffectCoordinates[higherIds + 1];
					FileSystemRequest.soundEffectVolumes[higherIds] = FileSystemRequest.soundEffectVolumes[higherIds + 1];
				}
				id--;
			} else {
				SoundEffect soundEffect = Class109.soundEffects[id];
				if (soundEffect == null) {
					soundEffect = SoundEffect.list(Class159.aClass50_1490, Class120_Sub12_Sub16.soundEffectIds[id], 0);
					if (soundEffect == null) {
						continue;
					}
					Class81.soundEffectDelays[id] += soundEffect.delay();
					Class109.soundEffects[id] = soundEffect;
				}
				if (Class81.soundEffectDelays[id] < 0) {
					int i_5_;
					if (Class174.SoundEffectCoordinates[id] != 0) {
						final int range = (Class174.SoundEffectCoordinates[id] & 0xff) * 128;
						final int z = Class174.SoundEffectCoordinates[id] >> 8 & 0xff;
						final int x = Class174.SoundEffectCoordinates[id] >> 16 & 0xff;
						int i_8_ = x * 128 + 64 - TileParticleQueue.selfPlayer.x;
						if (i_8_ < 0) {
							i_8_ = -i_8_;
						}
						int i_10_ = z * 128 + 64 - TileParticleQueue.selfPlayer.z;
						if (i_10_ < 0) {
							i_10_ = -i_10_;
						}
						...
						i_5_ = (range - i_11_) * CursorType.ambientSoundsVolume * FileSystemRequest.soundEffectVolumes[id] / range >> 8;
					} else {
						i_5_ = FileSystemRequest.soundEffectVolumes[id] * Class111.soundEffectVolume >> 8;
					}
					if (i_5_ > 0) {
						final Class120_Sub5_Sub1 class120_sub5_sub1 = soundEffect.method104().method1067(ObjectPile.aClass172_1809);
						final Class120_Sub30_Sub4 class120_sub30_sub4 = Class120_Sub30_Sub4.method1821(class120_sub5_sub1, 100, i_5_);
						class120_sub30_sub4.method1795(LocType.soundEffectLoops[id] - 1);
		...
		if (!Class120_Sub12_Sub15.aBoolean3247 || World.method2193()) {
			if (RuntimeException_Sub1.musicVolume != 0 && Class157.anInt1466 != -1 && !World.method2193()) {
			...
			if (RuntimeException_Sub1.musicVolume != 0 && Class157.anInt1466 != -1) {
				Class127.method1891(Class120_Sub12_Sub36.musicJs5, RuntimeException_Sub1.musicVolume, 0, Class157.anInt1466, false);
			}
	...
	static final void method1311() {
		...
		BasType.recentUse.clearSoftReference();
